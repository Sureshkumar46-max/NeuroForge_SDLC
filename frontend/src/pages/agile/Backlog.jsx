...
export default function Backlog() {
  ...
  return (
    <WorkspaceLayout
      title="Backlog"
      subtitle={`${backlogTasks.length} item${backlogTasks.length === 1 ? '' : 's'} awaiting sprint planning`}
      actions={
        <button
          className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-glow transition-transform duration-150 hover:scale-[1.02] hover:brightness-110"
    ...
    >
      ...
      <div className="surface-card overflow-hidden p-0">
        <div className="grid grid-cols-[90px_1fr_100px_90px_140px_90px] gap-3 border-b border-border bg-white/[0.02] px-4 py-3 text-[10.5px] font-semibold uppercase tracking-wide text-muted/70">
          <span>ID</span>
          <span>Title</span>
          <span>Priority</span>
          <span>Points</span>
        ...
        <div className="divide-y divide-border/60">
          {pageTasks.length === 0 ? (
            <div className="flex flex-col items-center gap-2 px-3 py-16 text-center">
              <ListTodo size={28} className="text-muted/40" />
              <p className="text-sm text-muted">No backlog items match this view.</p>
            </div>
          ) : (
